...
function detectEnvironment() {
  ...
  const hasCompose = existsSync('./docker-compose.yml') || existsSync('./docker-compose.full.yml');
  
  return {
    isDocker,
    hasCompose,
    inContainer: isDocker,
    hasSwarmNet: process.env.OLLAMA_HOST?.includes('ai_ollama')
  };
}
...
function displaySetupInstructions() {
  ...
  console.log('📋 Environment Setup Information:');
  console.log('═══════════════════════════════════════════════\n');
  
  if (env.inContainer) {
    console.log('🐳 Running in Docker container');
    console.log('✅ Containerized setup detected');
  } else if (env.hasCompose) {
    console.log('🐳 Docker Compose available');
    console.log('💡 To run in containerized mode:');
    console.log('   docker-compose -f docker-compose.full.yml up --profile demo');
  } else {
    ...
    console.log('   4. Set COURTLISTENER_API_TOKEN in .env');
  }
...
console.log(`   • Environment: ${env.inContainer ? 'Container' : 'Local'}`);
console.log(`   • API Token: ${process.env.COURTLISTENER_API_TOKEN ? '✅ Set' : '❌ Missing'}`);
console.log('');
...
async function setupOllama() {
  ...
  try {
    ...
    if (env.inContainer) {
      ...
      console.log('   • Check ai_ollama service is running');
      console.log('   • Verify swarm_net network connectivity');
      console.log('   • Check OLLAMA_HOST environment variable');
      console.log('   • Run: docker ps | grep ollama');
    } else if (env.hasCompose) {
      console.log('   🐳 Docker Compose Available:');
      console.log('   • Run full setup: docker-compose -f docker-compose.full.yml up');
      console.log('   • Check services: docker-compose -f docker-compose.full.yml ps');
    } else {
      console.log('   💻 Local Environment:');
      console.log('   • Check Ollama is running: ollama serve');
      console.log('   • Test connection: curl http://localhost:11434/api/tags');
      console.log('   • Install Ollama: https://ollama.ai/');
    }
    
    console.log(`   • Current host: ${OLLAMA_HOST}`);
    return false;
  }
}
